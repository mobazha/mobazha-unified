...
| ID   | 任务            | 状态      | 完成日期                                                                                                                        |
| ---- | --------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------- |
| M4-1 | JS 包瘦身       | ✅ 完成   | 首屏 1,163 KB → 504 KB gzip（-57%）：React.lazy(AppKitProvider/ChatSystem) + manualChunks 10 组 vendor 分包 + sideEffects:false |
| M4-2 | 字体按需加载    | ✅ 完成   | fonts.ts 重构：去除 next/font/google 依赖 → 纯运行时 loadStoreFont()；Inter 由 index.html 预加载，其他 7 字体按需注入           |
| M4-3 | API 请求优化    | ⏳ 未开始 |                                                                                                                                 |
| M4-4 | 列表虚拟化      | ⏳ 未开始 |                                                                                                                                 |
