...
> 版本: 2.3 | 日期: 2026-02-28 | 状态: **Implemented** (Phase 1-6 + 优化全部完成) 工作分支:
> `feature/pg-tier2-branding` Worktree: `mobazha-unified-tier2` 变更记录: v2.3
> — 优化完成（@dnd-kit、next/font、SSR SEO、响应式预览）；v2.2 — 标注实施完成状态；v2.1
> — 二次审核修复
>
> **实施说明**：Phase 1-6 核心功能 + 优化全部实现。
>
> - ✅ SSR SEO — `generateMetadata` 从 storefront config 提取 hero 标题/描述/theme-color
> - ✅ Google Fonts — `next/font/google` 预加载，消除 FOUT
> - ✅ Section 拖拽排序 — `@dnd-kit` 替代上下按钮
> - ✅ 编辑器响应式预览 — desktop/tablet/mobile 视口切换
> - 待后续：Store 页面全量 Server Component 重构、Profile.Colors fallback
...
现有店铺的 Tab 结构（Reviews / Following / Followers）在 Section 模式下通过 `store-tabs`
Section 承载：
...
**关键**：`storefrontConfig` 在服务端 fetch，Section 内容直接渲染在 HTML 中。不走 React
Query 客户端 fetch。
...
所有模板自动包含 `StoreTabs` Section（可由卖家隐藏）。模板中的文字自动从 Profile 填充（name → Hero
title, shortDescription → subtitle, about → About text）。
...
V1 MVP 简化实现：保存即发布（`status` 字段预留但固定为
`published`），Preview 按钮打开当前店铺页新标签。
...
| 1.8 | 后端 handlers + JSON 校验 + 路由              | mobazha        | `storefront_handlers.go`, 3 端点 |
...
现有 Profile 的 `Colors`
字段（Primary/Secondary/Text/Highlight/HighlightText）已有部分主题数据。策略：
...
最后更新: 2026-02-28 (v2.3: 优化完成 — @dnd-kit 拖拽排序、next/font 预加载、SSR
SEO、响应式预览; 更新 MVP 简化项状态)
