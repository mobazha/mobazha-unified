...
| 数字商品       | 80%          | 上传、文件管理                                                                |
| 通知系统       | 75%          | 多类型通知、已读/删除                                                         |
| 购物车→结账    | 30%          | **Cart 页面用 mock 数据，checkout 不接受 cart items**                         |
| 搜索           | 40%          | 店内搜索有，全局商品搜索缺失                                                  |
| 评价系统       | 30%          | 能显示不能提交                                                                |
| SEO/社交分享   | 5%           | 零 OG 标签、零 per-page meta                                                  |
| 卖家 Dashboard | 0%           | 完全没有                                                                      |
| 店铺品牌化     | 10%          | 仅头像+封面+简介                                                              |
| 移动端交易流   | 50%          | 框架有（MobileNav/BottomBar），但结账缺头部、Cart 用 mock、创建表单无移动导航 |
...
| `packages/core/services/ai/streaming.ts`          | 流式输出处理                  | PG-110（商品创建） |
| `apps/web/src/components/ai/AIGenerateButton.tsx` | 通用"AI 生成"按钮组件         | PG-110             |
| `apps/web/src/components/ai/AIStreamingText.tsx`  | 流式文字展示组件              | PG-110             |
...
最后更新: 2026-02-25 (v7: 新增 Mobile App(mobazha-mobile)设计参考映射(Section 2.3 + 8.1a)；历史项目参考原则统一为"看意图，重新设计")
