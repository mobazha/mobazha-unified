...
| **PG-109** | ✅ 折扣系统          | Shopify 风格独立折扣系统：后端 6 步（DiscountStore + AppService + Engine + API）+ 前端 3 步（结账集成 + 商品标签 + 卖家管理页）+ 46 后端测试        | 设计文档 `DISCOUNT_SYSTEM_DESIGN.md` | 已完成 |
...
| ID         | 任务                           | 说明                                                                                        | AI 增强 🤖                     | 预估      |
| ---------- | ------------------------------ | ------------------------------------------------------------------------------------------- | ------------------------------ | --------- |
| **PG-201** | ✅ 店铺品牌化（Section-based） | 12 Section 组件 + JSON 配置 + CSS 变量主题 + Admin 编辑器 + 5 预设模板 + 后端 API + 52 测试 | AI 推荐色板（从商品图提取）    | ✅ 已完成 |
...
| PG-109 | 折扣系统                  | V1.3  | 🔶 P1 后端 + Collection 集成完成 | 2026-02-28 P1 后端：Shopify 风格 Discount 系统（3 模型 + 4 层架构 + 11 API + 46 测试）+ Collection x Discount 集成（DI 注入 + 前端 DiscountForm collection selector），P2 结账前端待执行 |
...
| ID     | 任务                          | 状态      | 完成日期                                                                                                                                                      |
| ------ | ----------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PG-201 | 店铺品牌化（Section-based）   | ✅ 完成   | 2026-02-28 Phase 1-6 + 优化：12 Section + Theme + Admin Editor + 5 Presets + Backend API + i18n + 52 tests + @dnd-kit 拖拽 + next/font + SSR SEO + 响应式预览 |
...
最后更新: 2026-02-28 (v37: PG-201 优化完成 — @dnd-kit 拖拽排序、next/font 预加载、SSR SEO generateMetadata、响应式视口预览; 店铺品牌化 80%→95%)
