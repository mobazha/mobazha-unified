...
const FOOTER_CRYPTO_TOKENS_COMMUNITY = getEditionAdvertisedPaymentTokens();
...
const FOOTER_FIAT_METHODS: { id: string; name: string; icon: React.ReactNode }[] = [
  {
    ...
    icon: (
      <svg
        className="h-4 w-auto text-[#1A1F71] dark:text-[#9CA3C2]"
        viewBox="0 0 24 24"
        fill="currentColor"
        aria-hidden="true"
      >
  ...
  },
  {
    ...
    name: 'Mastercard',
    icon: (
      <svg className="h-5 w-auto" viewBox="0 0 24 16" fill="none" aria-hidden="true">
  ...
  },
  {
    ...
    icon: (
      <svg
        className="h-4 w-auto text-[#003087] dark:text-[#7B9FD4]"
        viewBox="0 0 24 24"
        fill="currentColor"
        aria-hidden="true"
      >
...
export const Footer: React.FC = () => {
  ...
  const footerCryptoTokens = isOutpostMode()
    ? FOOTER_CRYPTO_TOKENS_OUTPOST
    : runtimeConfig.schemaVersion >= 2
      ? FOOTER_CRYPTO_TOKENS_COMMUNITY.filter(token =>
          supportsRuntimePaymentMethod(token.id, 'crypto')
        )
      : FOOTER_CRYPTO_TOKENS_COMMUNITY;
