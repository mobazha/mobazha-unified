...
export interface ChatOrderThreadHeader {
  orderId: string;
  productTitle?: string;
  statusLabel?: string;
...
export interface ChatMessagesProps {
  ...
  onCall?: () => void;
  orderThread?: ChatOrderThreadHeader;
}
...
export const ChatMessages: React.FC<ChatMessagesProps> = ({
  ...
  const headerTitle = orderThread?.productTitle || roomName;
  const displayName = useMemo(() => cleanDisplayName(headerTitle), [headerTitle]);
  const headerSubtitle = orderThread
    ? [
        orderThread.statusLabel,
        orderThread.counterpartLabel
          ? t('order.chat.withName', { name: orderThread.counterpartLabel })
          : null,
      ]
        .filter(Boolean)
        .join(' · ')
  ...
  const backButton =
    onBack && !isEmbeddedApp ? (
      <button
        onClick={onBack}
        className="p-2 -ml-2 hover:bg-surface-hover rounded-lg transition-colors shrink-0"
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-background to-muted/20">
      ...
      {orderThread ? (
        <div
          className="shrink-0 bg-card/80 backdrop-blur-sm border-b border-border/50 shadow-sm"
        ...
        >
          <div className="flex items-center gap-2 px-3 pt-3 pb-2">
            ...
            <div className="flex-1 min-w-0">
              <button
                onClick={onRoomSettings}
                className="hover:opacity-80 transition-opacity text-left w-full min-w-0"
          ...
          <div className="flex items-center gap-2 px-3 pb-3">
            <p className="text-[12px] text-muted-foreground truncate flex-1 min-w-0">
              {headerSubtitle || t('order.chat.discussion')}
              {isEncrypted && <span className="text-primary/70"> · {t('chat.encrypted')}</span>}
            </p>
            {orderThread.onViewOrder && (
              <button
                type="button"
                onClick={orderThread.onViewOrder}
                className="shrink-0 inline-flex items-center gap-0.5 text-xs font-semibold text-primary hover:text-primary/80 transition-colors whitespace-nowrap"
                data-testid="chat-view-order-btn"
              >
                {t('order.chat.viewOrder')}
