...
const ACK_STORAGE_ITEM = 'mobazha-operator-responsibilities-ack-v1';
...
function readAckRaw(): string | null {
  ...
  try {
    return window.localStorage.getItem(ACK_STORAGE_ITEM);
  } catch {
    return null;
  }
}
...
function writeAck(record: AckRecord | null) {
  ...
  try {
    if (record === null) {
      window.localStorage.removeItem(ACK_STORAGE_ITEM);
    } else {
      window.localStorage.setItem(ACK_STORAGE_ITEM, JSON.stringify(record));
    }
  } catch {
