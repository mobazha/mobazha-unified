...
const COMMUNITY_CHAIN_ECOSYSTEM = getEditionAdvertisedPaymentTokens();
  ...
  ({ storeCount, listingCount, isLoading }) => {
    ...
    const chainEcosystem =
      runtimeConfig.schemaVersion >= 2
        ? COMMUNITY_CHAIN_ECOSYSTEM.filter(chain =>
            supportsRuntimePaymentMethod(chain.id, 'crypto')
          )
        : COMMUNITY_CHAIN_ECOSYSTEM;
        ...
        <Container size="xl">
          ...
          <div className="md:hidden text-center">
            ...
            <p className="text-white/20 text-[10px] mt-3">
              {t('saasHome.stats.utxoPayments', { count: chainsSupported })}
            </p>
          </div>
        </Container>
      </section>
