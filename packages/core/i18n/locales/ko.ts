/**
 * 한국어 번역
 */

import type { TranslationResource } from '../types';

export const ko: TranslationResource = {
  common: {
    loading: '로딩 중...',
    error: '오류',
    success: '성공',
    cancel: '취소',
    confirm: '확인',
    accept: '수락',
    decline: '거절',
    save: '저장',
    delete: '삭제',
    edit: '편집',
    back: '뒤로',
    next: '다음',
    search: '검색',
    filter: '필터',
    sort: '정렬',
    all: '전체',
    none: '없음',
    more: '더 보기',
    less: '접기',
    yes: '예',
    no: '아니오',
    ok: '확인',
    close: '닫기',
    retry: '재시도',
    refresh: '새로고침',
    submit: '제출',
    reset: '초기화',
    clear: '지우기',
    copy: '복사',
    copied: '복사됨!',
    share: '공유',
    download: '다운로드',
    upload: '업로드',
    view: '보기',
    viewAll: '전체 보기',
    seeMore: '더 보기',
    showMore: '더 표시',
    showLess: '간략히',
    noData: '데이터가 없습니다',
    backToMarket: '마켓으로 돌아가기',
    loadMore: '더 불러오기',
    noMoreData: '더 이상 데이터가 없습니다',
    comingSoon: '곧 출시 예정',
  },

  nav: {
    home: '홈',
    search: '검색',
    messages: '메시지',
    orders: '주문',
    profile: '프로필',
    settings: '설정',
    wallet: '지갑',
    cart: '장바구니',
    notifications: '알림',
    login: '로그인',
    logout: '로그아웃',
  },

  home: {
    welcome: 'Mobazha에 오신 것을 환영합니다',
    featured: '추천 상품',
    trending: '인기 상품',
    newArrivals: '신상품',
    categories: '카테고리',
    viewAll: '전체 보기',
  },

  product: {
    title: '제목',
    description: '설명',
    price: '가격',
    quantity: '수량',
    stock: '재고',
    inStock: '개 재고',
    outOfStock: '품절',
    category: '카테고리',
    condition: '상태',
    shipping: '배송',
    freeShipping: '무료 배송',
    shippingAtCheckout: '결제 시 배송비 계산',
    estDelivery: '예상 배송',
    returns: '반품',
    seller: '판매자',
    reviews: '개 리뷰',
    rating: '평점',
    addToCart: '장바구니에 추가',
    addedToCart: '장바구니에 추가됨',
    buyNow: '바로 구매',
    share: '공유',
    report: '신고',
    options: '옵션',
    selectOption: '옵션 선택',
    variants: '옵션',
    notFound: '상품을 찾을 수 없습니다',
    viewStore: '스토어 보기',
    acceptedCurrencies: '결제 가능',
    termsAndConditions: '이용약관',
    refundPolicy: '환불 정책',
    tags: '태그',
    noReviews: '아직 리뷰가 없습니다',
    viewAllReviews: '모든 리뷰 보기',
    anonymous: '익명',
  },

  search: {
    placeholder: '상품, 스토어 검색...',
    results: '{{count}}개의 결과',
    noResults: '결과 없음',
    filters: '필터',
    sortBy: '정렬 기준',
    relevance: '관련성',
    priceLowHigh: '가격 낮은순',
    priceHighLow: '가격 높은순',
    newest: '최신순',
    bestRating: '평점순',
    listings: '상품',
    users: '사용자',
  },

  cart: {
    title: '장바구니',
    empty: '장바구니가 비어 있습니다',
    emptyMessage: '아직 장바구니에 추가된 상품이 없습니다.',
    continueShopping: '쇼핑 계속하기',
    subtotal: '소계',
    shipping: '배송비',
    total: '총액',
    checkout: '결제하기',
    remove: '삭제',
    quantity: '수량',
    itemCount: '{{count}}개 상품',
    itemsInCart: '장바구니에 {{count}}개 상품',
    selectAll: '전체 선택',
    deselectAll: '전체 해제',
    orderSummary: '주문 요약',
    proceedToCheckout: '결제하기',
    free: '무료',
    acceptedPayments: '결제 가능한 수단',
    startShopping: '쇼핑 시작',
  },

  checkout: {
    title: '결제',
    shippingAddress: '배송 주소',
    paymentMethod: '결제 수단',
    orderSummary: '주문 요약',
    placeOrder: '주문하기',
    processing: '처리 중...',
    selectCrypto: '암호화폐 선택',
    paymentAddress: '결제 주소',
    scanQR: 'QR 코드 스캔',
    copyAddress: '주소 복사',
    amountToPay: '결제 금액',
    waitingPayment: '결제 대기 중',
    paymentConfirmed: '결제 확인됨',
  },

  order: {
    title: '주문',
    myOrders: '내 주문',
    orderNumber: '주문번호 #{{number}}',
    status: '상태',
    date: '날짜',
    total: '총액',
    items: '상품',
    details: '주문 상세',
    tracking: '배송 추적',
    pending: '대기 중',
    confirmed: '확인됨',
    shipped: '발송됨',
    delivered: '배송 완료',
    completed: '완료',
    cancelled: '취소됨',
    refunded: '환불됨',
    disputed: '분쟁 중',
    purchases: '구매',
    sales: '판매',
    myPurchases: '내 구매',
    mySales: '내 판매',
    allOrders: '모든 주문',
    processing: '처리 중',
    manageOrders: '구매 및 판매 관리',
    noOrdersFound: '주문을 찾을 수 없습니다',
    noOrdersMessage: '아직 {{type}}이(가) 없습니다. 마켓플레이스를 둘러보세요!',
    noStatusOrders: '현재 {{status}} 주문이 없습니다.',
    // Order detail page
    backToOrders: '주문 목록으로',
    placedOn: '{{date}}에 주문됨',
    orderNotFound: '주문을 찾을 수 없음',
    orderNotFoundMessage: '찾으시는 주문이 존재하지 않습니다.',
    loadOrderFailed: '주문 로드 실패',
    tryAgain: '다시 시도',
    message: '메시지',
    confirmReceipt: '수령 확인',
    confirmingReceipt: '처리 중...',
    openDispute: '분쟁 열기',
    markAsShipped: '발송 완료로 표시',
    refundOrder: '환불',
    shipOrder: '배송하기',
    // Timeline
    orderTimeline: '주문 타임라인',
    orderPlaced: '주문 접수',
    paymentConfirmed: '결제 확인',
    vendorConfirmed: '판매자 확인',
    packageShipped: '배송 시작',
    orderCompleted: '주문 완료',
    fundsReleased: '대금 지급 완료',
    disputeOpened: '분쟁 시작',
    disputeResolved: '분쟁 해결',
    // Order items
    orderItems: '주문 상품',
    quantity: '수량',
    subtotal: '소계',
    shipping: '배송',
    free: '무료',
    moderatorFee: '중재자 수수료',
    // Parties
    seller: '판매자',
    buyer: '구매자',
    moderator: '중재자',
    viewStore: '스토어 보기',
    // Address and payment
    shippingAddress: '배송 주소',
    trackingNumber: '운송장 번호',
    paymentDetails: '결제 정보',
    paymentTransaction: '결제 트랜잭션',
    escrowAddress: '에스크로 주소',
    orderNotes: '주문 메모',
    // Dispute
    disputeOpen: '분쟁 진행 중',
    initiatedBy: '{{party}}이(가) 시작',
    disputeStatus: '상태: {{status}}',
    favorBuyer: '구매자 지지',
  },

  chat: {
    title: '메시지',
    conversations: '대화',
    newMessage: '새 메시지',
    typeMessage: '메시지 입력...',
    send: '보내기',
    noMessages: '메시지 없음',
    online: '온라인',
    offline: '오프라인',
    lastSeen: '{{time}} 전 접속',
    typing: '입력 중...',
    encrypted: '메시지는 암호화되어 있습니다',
    welcomeToMessages: '메시지에 오신 것을 환영합니다',
    selectConversation:
      '대화를 선택하여 채팅을 시작하거나 새 대화를 만들어 판매자 및 구매자와 연결하세요.',
    searchConversations: '대화 검색...',
    // 섹션
    directMessages: '다이렉트 메시지',
    communities: '커뮤니티',
    orderChats: '주문 토론',
    invitations: '초대',
    invitedYou: '채팅에 초대했습니다',
    inviteConfirm: '초대',
    invitedBy: '{{name}}의 초대',
    // 방 설정
    roomSettings: '방 설정',
    roomId: '방 ID',
    members: '멤버',
    directMessage: '다이렉트 메시지',
    groupChat: '그룹 채팅',
    viewStore: '상점 보기',
    sendMessage: '메시지 보내기',
    // 사용자 정보
    unknownUser: '알 수 없는 사용자',
    externalUser: '외부 Matrix 사용자',
    peerId: '피어 ID',
    matrixId: 'Matrix ID',
    // 시간
    today: '오늘',
    yesterday: '어제',
    connecting: '채팅에 연결 중...',
    startConversation: '대화 시작',
  },

  // Matrix 房间事件
  matrix: {
    events: {
      join: '{{name}}님이 채팅에 참여했습니다',
      left: '{{name}}님이 채팅에서 나갔습니다',
      invited: '{{by}}님이 {{name}}님을 초대했습니다',
      kicked: '{{by}}님이 {{name}}님을 채팅에서 제거했습니다',
      banned: '{{by}}님이 {{name}}님을 차단했습니다',
      unbanned: '{{by}}님이 {{name}}님의 차단을 해제했습니다',
      nameChanged: '{{name}}님이 이름을 {{newName}}로 변경했습니다',
      avatarChanged: '{{name}}님이 아바타를 변경했습니다',
      roomNameChanged: '{{name}}님이 방 이름을 {{roomName}}로 변경했습니다',
      roomTopicChanged: '{{name}}님이 주제를 {{topic}}로 설정했습니다',
      encryptionEnabled: '종단간 암호화가 활성화되었습니다',
      roomCreated: '{{name}}님이 이 채팅을 생성했습니다',
    },
  },

  wallet: {
    title: '지갑',
    balance: '잔액',
    totalBalance: '총 잔액',
    send: '보내기',
    receive: '받기',
    transactions: '거래 내역',
    address: '주소',
    amount: '금액',
    fee: '수수료',
    memo: '메모',
    confirm: '거래 확인',
    history: '거래 내역',
    noTransactions: '거래 내역 없음',
    sent: '보냄',
    received: '받음',
    pending: '대기 중',
    totalPortfolioValue: '총 포트폴리오 가치',
    exchange: '교환',
    yourWallets: '내 지갑',
    yourAssets: '내 자산',
    transactionHistory: '거래 내역',
    clear: '지우기',
    all: '전체',
    today: '오늘',
    back: '뒤로',
    yourAddress: '내 주소',
    copy: '복사',
    connect: '지갑 연결',
    connecting: '연결 중...',
    disconnect: '연결 해제',
    connected: '연결됨',
  },

  payment: {
    title: '결제',
    selectPaymentMethod: '결제 방법 선택',
    selectPaymentMethodDesc: '이 주문에 대한 결제 방법을 선택하세요',
    paymentMethod: '결제 방법',
    otherMethods: '다른 결제 방법',
    showMore: '더 보기',
    showLess: '접기',
    noTokensAvailable: '이용 가능한 결제 방법이 없습니다',
    paymentProtection: '결제 보호',
    paymentProtectionInfo:
      '결제 보호를 통해 중재자가 구매자와 판매자 간의 분쟁을 해결할 수 있습니다.',
    protectFor: '최대 {{days}}일 동안 결제 보호',
    selectModerator: '중재자 선택',
    selectModeratorDesc: '결제 보호를 위한 중재자를 선택하세요',
    selectModeratorWarning: '에스크로 보호를 위한 중재자를 선택해 주세요',
    selectPaymentMethodWarning: '결제 방법을 선택해 주세요',
    changeModerator: '변경',
    enableProtection: '보호 활성화',
    disableProtection: '보호 비활성화',
    fee: '수수료',
    noOrderID: '주문 ID가 제공되지 않았습니다',
    loadOrderFailed: '주문 세부 정보를 불러오지 못했습니다',
    errorTitle: '주문을 불러올 수 없습니다',
    noOrderData: '주문 데이터가 없습니다',
    paymentSummary: '결제 요약',
    orderTotal: '주문 합계',
    moderatorFee: '중재자 수수료',
    totalToPay: '총 결제 금액',
    pay: '결제',
    connectWallet: '결제하려면 지갑 연결',
    connecting: '연결 중...',
    processing: '처리 중...',
    success: '결제 성공!',
    failed: '결제 실패: ',
    securityNote: '멀티시그 에스크로 보호로 안전한 결제',
  },

  profile: {
    title: '프로필',
    myProfile: '내 프로필',
    editProfile: '프로필 수정',
    about: '소개',
    location: '위치',
    joined: '{{date}} 가입',
    followers: '팔로워',
    following: '팔로잉',
    listings: '상품',
    reviews: '리뷰',
    verified: '인증됨',
    store: '스토어',
    otc: 'OTC',
    posts: '게시물',
    community: '커뮤니티',
    viewStore: '스토어 보기',
    follow: '팔로우',
    unfollow: '언팔로우',
    message: '메시지',
    block: '차단',
    report: '신고',
    myStore: '내 스토어',
    addNewListing: '새 상품 등록',
    contactInformation: '연락처 정보',
    email: '이메일',
    phone: '전화번호',
    website: '웹사이트',
    profileInformation: '프로필 정보',
    name: '이름',
    bio: '자기소개',
    saveChanges: '변경사항 저장',
    noProfileData: '프로필 데이터를 불러올 수 없습니다',
  },

  settings: {
    title: '설정',
    general: '일반',
    account: '계정',
    notifications: '알림',
    privacy: '개인정보',
    security: '보안',
    language: '언어',
    currency: '통화',
    theme: '테마',
    darkMode: '다크 모드',
    lightMode: '라이트 모드',
    system: '시스템',
    logout: '로그아웃',
    deleteAccount: '계정 삭제',
    backup: '백업',
    restore: '복원',
    about: '정보',
    version: '버전',
    termsOfService: '서비스 약관',
    privacyPolicy: '개인정보 처리방침',
    appearance: '외관',
    chooseTheme: '테마 선택',
    displayMode: '표시 모드',
    avatar: '아바타',
    loadAvatar: '아바타 변경',
    loadHeader: '커버 변경',
  },

  theme: {
    classic: '클래식 시안',
    classicDesc: 'Mobazha 브랜드 클래식 색상, 상쾌하고 활기찬',
    crypto: '크립토 나이트',
    cryptoDesc: '네온 그린의 어두운 배경, 암호화폐 트레이더용',
    business: '비즈니스 블루',
    businessDesc: '전문적인 블루 톤, 공식 비즈니스용',
    cyberpunk: '사이버펑크',
    cyberpunkDesc: '퍼플-핑크 그라데이션, NFT와 디지털 아트에 적합',
    nature: '트러스트 오아시스',
    natureDesc: '포레스트 그린 톤, 신뢰와 지속 가능성 전달',
    luxury: '럭셔리 골드',
    luxuryDesc: '블랙과 골드, 럭셔리 상품 거래용',
  },

  errors: {
    generic: '오류가 발생했습니다',
    network: '네트워크 오류입니다. 연결을 확인하세요.',
    notFound: '찾을 수 없음',
    unauthorized: '로그인이 필요합니다',
    forbidden: '접근이 거부되었습니다',
    serverError: '서버 오류입니다. 나중에 다시 시도하세요.',
    timeout: '요청 시간이 초과되었습니다',
    offline: '오프라인 상태입니다',
    invalidInput: '잘못된 입력',
    requiredField: '필수 항목입니다',
  },

  time: {
    justNow: '방금',
    minutesAgo: '{{count}}분 전',
    hoursAgo: '{{count}}시간 전',
    daysAgo: '{{count}}일 전',
    weeksAgo: '{{count}}주 전',
    monthsAgo: '{{count}}개월 전',
    yearsAgo: '{{count}}년 전',
  },

  hero: {
    badge: '탈중앙화 & P2P',
    titleLine1: '자유롭게 거래하세요.',
    titleLine2: '안전하게 거래하세요.',
    subtitle:
      '디지털 상품, 서비스, 암호화폐 OTC 거래를 위한 진정한 탈중앙화 마켓플레이스. 중개자 없음. 검열 없음.',
    exploreMarket: '마켓 탐색',
    startSelling: '판매 시작',
    activeStores: '활성 스토어',
    productsListed: '등록된 상품',
    platformFee: '플랫폼 수수료',
  },

  footer: {
    tagline: '미래 상거래를 위한 탈중앙화 마켓플레이스.',
    marketplace: '마켓플레이스',
    browseProducts: '상품 둘러보기',
    categories: '카테고리',
    findModerators: '중재자 찾기',
    startSelling: '판매 시작',
    resources: '리소스',
    gettingStarted: '시작하기',
    documentation: '문서',
    api: 'API',
    faq: 'FAQ',
    company: '회사',
    about: '소개',
    blog: '블로그',
    careers: '채용',
    contact: '문의',
    allRightsReserved: 'All rights reserved.',
  },

  marketplace: {
    title: '커뮤니티 마켓플레이스',
    subtitle: '커뮤니티 기반 마켓플레이스를 발견하고 참여하세요.',
    searchPlaceholder: '마켓플레이스 검색...',
    featured: '추천',
    featuredMarketplaces: '추천 마켓플레이스',
    allMarketplaces: '모든 마켓플레이스',
    searchResults: '검색 결과',
    createMarketplace: '마켓플레이스 만들기',
    members: '멤버',
    sellers: '판매자',
    products: '상품',
    noMarketplacesFound: '마켓플레이스를 찾을 수 없습니다',
    clearFilters: '필터 지우기',
    sortByMembers: '멤버 순',
    sortByProducts: '상품 순',
    sortByName: '이름 순',
    allCategories: '모든 카테고리',
  },

  moderator: {
    title: '중재자',
    subtitle: '신뢰할 수 있는 중재자를 선택하여 에스크로로 거래를 보호하세요',
    searchPlaceholder: '중재자 검색...',
    language: '언어',
    allLanguages: '모든 언어',
    maxFee: '최대 수수료 (%)',
    anyFee: '수수료 제한 없음',
    verifiedOnly: '인증된 중재자만',
    sortBy: '정렬',
    highestRating: '평점 순',
    lowestFee: '수수료 낮은 순',
    mostDisputes: '분쟁 처리 순',
    resetFilters: '필터 초기화',
    moderatorsFound: '{{count}}명의 중재자를 찾았습니다',
    disputesHandled: '{{count}}건의 분쟁 처리',
    success: '{{rate}}% 성공률',
    fee: '수수료',
    noModeratorsFound: '조건에 맞는 중재자를 찾을 수 없습니다',
  },

  filter: {
    filters: '필터',
    category: '카테고리',
    priceRange: '가격 범위',
    min: '최소',
    max: '최대',
    rating: '평점',
    anyRating: '평점 제한 없음',
    stars: '{{count}}점 이상',
    type: '유형',
    allTypes: '모든 유형',
    physicalGoods: '실물 상품',
    digitalGoods: '디지털 상품',
    services: '서비스',
    rwaTokens: 'RWA 토큰',
  },

  empty: {
    noProductsFound: '상품을 찾을 수 없습니다',
    noStoresFound: '스토어를 찾을 수 없습니다',
    tryAdjustingSearch: '검색어를 조정해 보세요.',
    tryAdjustingFilters: '검색 또는 필터를 조정해 보세요.',
    noRecentSearches: '최근 검색 없음',
    loadMoreResults: '더 보기',
  },

  homeExtended: {
    trendingNow: '인기 상품',
    trendingSubtitle: '이번 주 인기 상품',
    featuredServices: '추천 & 서비스',
    featuredSubtitle: '엄선된 상품과 서비스',
    browseCategories: '카테고리 둘러보기',
    electronics: '전자제품',
    digitalGoods: '디지털 상품',
    services: '서비스',
    cryptoOtc: '암호화폐 OTC',
  },

  searchExtended: {
    searchPlaceholder: '상품, 스토어 검색 또는 URL 붙여넣기...',
    recentSearches: '최근 검색',
    clearAll: '모두 지우기',
    popularCategories: '인기 카테고리',
    products: '상품',
    stores: '스토어',
  },

  settingsExtended: {
    profile: '프로필',
    country: '국가',
    shippingAddresses: '배송 주소',
    manageAddresses: '배송 주소 관리',
    blockedUsers: '차단된 사용자',
    manageBlocked: '차단된 계정 관리',
    pushNotifications: '푸시 알림',
    pushDescription: '주문 및 메시지에 대한 푸시 알림 받기',
    emailNotifications: '이메일 알림',
    emailDescription: '이메일로 업데이트 받기',
    store: '스토어',
    privateStore: '비공개 스토어',
    privateStoreDesc: '승인된 사용자만 상품을 볼 수 있습니다',
    storePolicies: '스토어 정책',
    storePoliciesDesc: '반품 정책, 이용약관',
    moderators: '중재자',
    moderatorsDesc: '분쟁 중재자 관리',
    acceptedCrypto: '허용된 암호화폐',
    selected: '{{count}}개 선택됨',
    shippingOptions: '배송 옵션',
    shippingOptionsDesc: '배송 방법 및 가격 설정',
    advanced: '고급',
    analytics: '분석',
    analyticsDesc: '익명 사용 데이터를 공유하여 앱 개선에 도움',
    backupWallet: '지갑 백업',
    backupWalletDesc: '시드 문구 저장',
    backupProfile: '프로필 백업',
    backupProfileDesc: '프로필 데이터 내보내기',
    restoreProfile: '프로필 복원',
    restoreProfileDesc: '백업에서 프로필 가져오기',
    resyncTransactions: '거래 재동기화',
    resyncDesc: '블록체인 거래 재동기화',
    serverLogs: '서버 로그',
    serverLogsDesc: '애플리케이션 로그 보기',
    checkForUpdates: '업데이트 확인',
    upToDate: '최신 버전',
    latestVersion: '최신 버전을 사용 중입니다!',
    selectCountry: '국가 선택',
    selectCurrency: '통화 선택',
    currencyUpdated: '통화가 다음으로 업데이트됨',
    acceptedCryptocurrencies: '허용된 암호화폐',
    done: '완료',
    light: '라이트',
    dark: '다크',
    restoreConfirmTitle: '프로필 복원',
    restoreConfirmDesc:
      '복원하시겠습니까? 먼저 백업을 확인하세요. 이 작업으로 현재 프로필 데이터가 덮어씌워질 수 있습니다.',
    continue: '계속',
    logoutConfirmTitle: '로그아웃',
    logoutConfirmDesc: '로그아웃하시겠습니까? 다시 로그인해야 합니다.',
    comingSoon: '곧 출시',
    backupComingSoon: '지갑 백업 기능이 곧 출시됩니다!',
    restoreComingSoon: '프로필 복원 기능이 곧 출시됩니다!',
    followingSystem: '시스템 설정 따름',
    enabled: '활성화됨',
    disabled: '비활성화됨',
  },

  userMenu: {
    myProfile: '내 프로필',
    myStore: '내 스토어',
    createListing: '상품 등록',
    sales: '판매 주문',
    purchases: '구매 주문',
    settings: '설정',
    logout: '로그아웃',
  },

  // OTC 개인 거래
  otc: {
    title: 'OTC 거래',
    createNft: 'NFT 주문 생성',
    createErc3525: '지분 주문 생성',
    createNftOrder: 'NFT 개인 주문 생성',
    createOrder: '주문 생성',
    noOtcYet: 'OTC 자산이 없습니다',
    noOtcInStore: '이 스토어에는 OTC 자산이 없습니다',
    noOtcInMarketplace: '이 마켓에는 OTC 자산이 없습니다',
    createFirstOtc: '첫 번째 OTC 주문을 생성하여 개인 거래를 시작하세요',
    beFirstToCreate: '커뮤니티에서 첫 번째 OTC 주문을 생성하세요',
    activeOrders: '활성 주문',
    historyOrders: '기록',
    fetchError: 'OTC 주문을 가져오지 못했습니다',
    step: { selectNft: 'NFT 선택', setPrice: '가격 설정', shareLink: '링크 공유' },
    status: { active: '활성', completed: '완료됨', cancelled: '취소됨' },
    price: '가격', priceDesc: '구매자가 USDT로 결제합니다', shares: '지분', totalPrice: '총 가격', platformFeeNote: '* 플랫폼 수수료 5%', youWillReceive: '받을 금액', selectNftDesc: '판매할 NFT를 선택하세요',
    creating: '생성 중...', purchasing: '구매 중...', cancelling: '취소 중...', buyNow: '지금 구매', buyShares: '지분 구매', cancelOrder: '주문 취소', confirmCancel: '이 주문을 취소하시겠습니까?',
    createFailed: '주문 생성 실패', orderCreated: '주문이 생성되었습니다!', purchaseSuccess: '구매 성공!', purchaseFailed: '구매 실패', cancelSuccess: '주문이 취소되었습니다', cancelFailed: '취소 실패', orderNotFound: '주문을 찾을 수 없습니다', transactionCompleted: '거래 완료', orderCancelled: '주문 취소됨',
    shareText: '개인 NFT 자산을 판매 중입니다, 확인해 보세요!', shareToSell: '아래 링크를 구매자에게 공유하세요', shareToTelegram: 'Telegram에서 공유', viewOrder: '주문 보기', createAnother: '다른 주문 생성',
    contractInfo: '컨트랙트 정보', seller: '판매자', network: '네트워크', expectedRevenue: '예상 수익', weeklyRevenue: '주간 수익', annualizedRevenue: '연간 수익', chatWithSeller: '판매자와 채팅',
    connectWalletRequired: '먼저 지갑을 연결하세요', connectWalletDesc: 'OTC 주문을 생성하려면 지갑 연결이 필요합니다', backToStore: '스토어로 돌아가기',
    marketplaceOtcTitle: '🔐 개인 OTC 거래', marketplaceOtcDesc: '커뮤니티 멤버의 NFT 및 RWA 지분 목록',
  },

};

export default ko;
