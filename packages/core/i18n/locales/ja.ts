/**
 * 日本語翻訳
 */

import type { TranslationResource } from '../types';

export const ja: TranslationResource = {
  common: {
    loading: '読み込み中...',
    error: 'エラー',
    success: '成功',
    cancel: 'キャンセル',
    confirm: '確認',
    accept: '承認',
    decline: '拒否',
    save: '保存',
    delete: '削除',
    edit: '編集',
    back: '戻る',
    next: '次へ',
    search: '検索',
    filter: 'フィルター',
    sort: '並び替え',
    all: 'すべて',
    none: 'なし',
    more: 'もっと見る',
    less: '閉じる',
    yes: 'はい',
    no: 'いいえ',
    ok: 'OK',
    close: '閉じる',
    retry: '再試行',
    refresh: '更新',
    submit: '送信',
    reset: 'リセット',
    clear: 'クリア',
    copy: 'コピー',
    copied: 'コピーしました！',
    share: '共有',
    download: 'ダウンロード',
    upload: 'アップロード',
    view: '表示',
    viewAll: 'すべて見る',
    seeMore: 'もっと見る',
    showMore: '表示を増やす',
    showLess: '表示を減らす',
    noData: 'データがありません',
    backToMarket: 'マーケットに戻る',
    loadMore: 'もっと読み込む',
    noMoreData: 'これ以上データはありません',
    comingSoon: '近日公開',
  },

  nav: {
    home: 'ホーム',
    search: '検索',
    messages: 'メッセージ',
    orders: '注文',
    profile: 'プロフィール',
    settings: '設定',
    wallet: 'ウォレット',
    cart: 'カート',
    notifications: '通知',
    login: 'ログイン',
    logout: 'ログアウト',
  },

  home: {
    welcome: 'Mobazhへようこそ',
    featured: '注目の商品',
    trending: '人気商品',
    newArrivals: '新着商品',
    categories: 'カテゴリー',
    viewAll: 'すべて見る',
  },

  product: {
    title: 'タイトル',
    description: '説明',
    price: '価格',
    quantity: '数量',
    stock: '在庫',
    inStock: '点在庫あり',
    outOfStock: '在庫切れ',
    category: 'カテゴリー',
    condition: '状態',
    shipping: '配送',
    freeShipping: '送料無料',
    shippingAtCheckout: '送料はお会計時に計算',
    estDelivery: 'お届け予定',
    returns: '返品',
    seller: '販売者',
    reviews: '件のレビュー',
    rating: '評価',
    addToCart: 'カートに追加',
    addedToCart: 'カートに追加しました',
    buyNow: '今すぐ購入',
    share: '共有',
    report: '報告',
    options: 'オプション',
    selectOption: 'オプションを選択',
    variants: 'バリエーション',
    notFound: '商品が見つかりません',
    viewStore: 'ショップを見る',
    acceptedCurrencies: '対応通貨',
    termsAndConditions: '利用規約',
    refundPolicy: '返金ポリシー',
    tags: 'タグ',
    noReviews: 'まだレビューがありません',
    viewAllReviews: 'すべてのレビューを見る',
    anonymous: '匿名',
  },

  search: {
    placeholder: '商品、ストアを検索...',
    results: '{{count}}件の結果',
    noResults: '結果が見つかりません',
    filters: 'フィルター',
    sortBy: '並び替え',
    relevance: '関連性',
    priceLowHigh: '価格：低い順',
    priceHighLow: '価格：高い順',
    newest: '新着順',
    bestRating: '評価順',
    listings: '商品',
    users: 'ユーザー',
  },

  cart: {
    title: 'ショッピングカート',
    empty: 'カートは空です',
    emptyMessage: 'まだカートに商品がありません。',
    continueShopping: '買い物を続ける',
    subtotal: '小計',
    shipping: '送料',
    total: '合計',
    checkout: 'レジに進む',
    remove: '削除',
    quantity: '数量',
    itemCount: '{{count}}点の商品',
    itemsInCart: 'カートに{{count}}点の商品',
    selectAll: 'すべて選択',
    deselectAll: '選択を解除',
    orderSummary: '注文概要',
    proceedToCheckout: 'レジに進む',
    free: '無料',
    acceptedPayments: '利用可能な支払い方法',
    startShopping: 'ショッピングを始める',
  },

  checkout: {
    title: 'レジ',
    shippingAddress: '配送先住所',
    paymentMethod: '支払い方法',
    orderSummary: '注文概要',
    placeOrder: '注文を確定',
    processing: '処理中...',
    selectCrypto: '暗号通貨を選択',
    paymentAddress: '支払いアドレス',
    scanQR: 'QRコードをスキャン',
    copyAddress: 'アドレスをコピー',
    amountToPay: '支払い金額',
    waitingPayment: '支払い待ち',
    paymentConfirmed: '支払い確認済み',
  },

  order: {
    title: '注文',
    myOrders: '注文履歴',
    orderNumber: '注文番号 #{{number}}',
    status: 'ステータス',
    date: '日付',
    total: '合計',
    items: '商品',
    details: '注文詳細',
    tracking: '追跡',
    pending: '保留中',
    confirmed: '確認済み',
    shipped: '発送済み',
    delivered: '配達済み',
    completed: '完了',
    cancelled: 'キャンセル',
    refunded: '返金済み',
    disputed: '紛争中',
    purchases: '購入',
    sales: '販売',
    myPurchases: '購入履歴',
    mySales: '販売履歴',
    allOrders: 'すべての注文',
    processing: '処理中',
    manageOrders: '購入と販売を管理',
    noOrdersFound: '注文が見つかりません',
    noOrdersMessage: 'まだ{{type}}がありません。マーケットプレイスを探索してみましょう！',
    noStatusOrders: '現在{{status}}の注文はありません。',
    // Order detail page
    backToOrders: '注文一覧に戻る',
    placedOn: '{{date}}に注文',
    orderNotFound: '注文が見つかりません',
    orderNotFoundMessage: 'お探しの注文は存在しません。',
    loadOrderFailed: '注文の読み込みに失敗しました',
    tryAgain: '再試行',
    message: 'メッセージ',
    confirmReceipt: '受取確認',
    confirmingReceipt: '処理中...',
    openDispute: '紛争を開始',
    markAsShipped: '発送済みにする',
    refundOrder: '返金',
    shipOrder: '発送する',
    // Timeline
    orderTimeline: '注文タイムライン',
    orderPlaced: '注文確定',
    paymentConfirmed: '支払い確認',
    vendorConfirmed: '販売者確認',
    packageShipped: '発送完了',
    orderCompleted: '注文完了',
    fundsReleased: '資金解放',
    disputeOpened: '紛争開始',
    disputeResolved: '紛争解決',
    // Order items
    orderItems: '注文商品',
    quantity: '数量',
    subtotal: '小計',
    shipping: '配送',
    free: '無料',
    moderatorFee: 'モデレーター手数料',
    // Parties
    seller: '販売者',
    buyer: '購入者',
    moderator: 'モデレーター',
    viewStore: 'ストアを見る',
    // Address and payment
    shippingAddress: '配送先住所',
    trackingNumber: '追跡番号',
    paymentDetails: '支払い詳細',
    paymentTransaction: '支払いトランザクション',
    escrowAddress: 'エスクローアドレス',
    orderNotes: '注文メモ',
    // Dispute
    disputeOpen: '紛争中',
    initiatedBy: '{{party}}が開始',
    disputeStatus: 'ステータス: {{status}}',
    favorBuyer: '購入者有利',
  },

  chat: {
    title: 'メッセージ',
    conversations: '会話',
    newMessage: '新しいメッセージ',
    typeMessage: 'メッセージを入力...',
    send: '送信',
    noMessages: 'メッセージがありません',
    online: 'オンライン',
    offline: 'オフライン',
    lastSeen: '{{time}}前にオンライン',
    typing: '入力中...',
    encrypted: 'メッセージは暗号化されています',
    welcomeToMessages: 'メッセージへようこそ',
    selectConversation:
      '会話を選択してチャットを開始するか、新しい会話を作成して販売者や購入者と繋がりましょう。',
    searchConversations: '会話を検索...',
    // セクション
    directMessages: 'ダイレクトメッセージ',
    communities: 'コミュニティ',
    orderChats: '注文のディスカッション',
    invitations: '招待',
    invitedYou: 'チャットに招待しました',
    inviteConfirm: '招待',
    invitedBy: '{{name}}から招待',
    // ルーム設定
    roomSettings: 'ルーム設定',
    roomId: 'ルームID',
    members: 'メンバー',
    directMessage: 'ダイレクトメッセージ',
    groupChat: 'グループチャット',
    viewStore: 'ストアを見る',
    sendMessage: 'メッセージを送信',
    // ユーザー情報
    unknownUser: '不明なユーザー',
    externalUser: '外部Matrixユーザー',
    peerId: 'ピアID',
    matrixId: 'Matrix ID',
    // 時間
    today: '今日',
    yesterday: '昨日',
    connecting: 'チャットに接続中...',
    startConversation: '会話を始める',
  },

  // Matrix 房间事件
  matrix: {
    events: {
      join: '{{name}} がチャットに参加しました',
      left: '{{name}} がチャットから退出しました',
      invited: '{{by}} が {{name}} を招待しました',
      kicked: '{{by}} が {{name}} をチャットから削除しました',
      banned: '{{by}} が {{name}} をブロックしました',
      unbanned: '{{by}} が {{name}} のブロックを解除しました',
      nameChanged: '{{name}} が名前を {{newName}} に変更しました',
      avatarChanged: '{{name}} がアバターを変更しました',
      roomNameChanged: '{{name}} がルーム名を {{roomName}} に変更しました',
      roomTopicChanged: '{{name}} がトピックを {{topic}} に設定しました',
      encryptionEnabled: 'エンドツーエンド暗号化が有効になりました',
      roomCreated: '{{name}} がこのチャットを作成しました',
    },
  },

  wallet: {
    title: 'ウォレット',
    balance: '残高',
    totalBalance: '総残高',
    send: '送金',
    receive: '受取',
    transactions: '取引履歴',
    address: 'アドレス',
    amount: '金額',
    fee: '手数料',
    memo: 'メモ',
    confirm: '取引を確認',
    history: '取引履歴',
    noTransactions: '取引履歴がありません',
    sent: '送金済み',
    received: '受取済み',
    pending: '保留中',
    totalPortfolioValue: 'ポートフォリオ総額',
    exchange: '交換',
    yourWallets: 'ウォレット',
    yourAssets: '資産一覧',
    transactionHistory: '取引履歴',
    clear: 'クリア',
    all: 'すべて',
    today: '今日',
    back: '戻る',
    yourAddress: 'あなたのアドレス',
    copy: 'コピー',
    connect: 'ウォレットを接続',
    connecting: '接続中...',
    disconnect: '切断',
    connected: '接続済み',
  },

  payment: {
    title: '支払い',
    selectPaymentMethod: '支払い方法を選択',
    selectPaymentMethodDesc: 'この注文の支払い方法を選択してください',
    paymentMethod: '支払い方法',
    otherMethods: 'その他の支払い方法',
    showMore: 'もっと見る',
    showLess: '閉じる',
    noTokensAvailable: '利用可能な支払い方法がありません',
    paymentProtection: '支払い保護',
    paymentProtectionInfo:
      '支払い保護により、モデレーターが買い手と売り手の間の紛争を解決できます。',
    protectFor: '最大{{days}}日間支払いを保護',
    selectModerator: 'モデレーターを選択',
    selectModeratorDesc: '支払い保護のモデレーターを選択してください',
    selectModeratorWarning: 'エスクロー保護のモデレーターを選択してください',
    selectPaymentMethodWarning: '支払い方法を選択してください',
    changeModerator: '変更',
    enableProtection: '保護を有効にする',
    disableProtection: '保護を無効にする',
    fee: '手数料',
    noOrderID: '注文IDが提供されていません',
    loadOrderFailed: '注文詳細の読み込みに失敗しました',
    errorTitle: '注文を読み込めません',
    noOrderData: '注文データがありません',
    paymentSummary: '支払い概要',
    orderTotal: '注文合計',
    moderatorFee: 'モデレーター手数料',
    totalToPay: '支払い総額',
    pay: '支払う',
    connectWallet: 'ウォレットを接続して支払う',
    connecting: '接続中...',
    processing: '処理中...',
    success: '支払い成功！',
    failed: '支払い失敗: ',
    securityNote: 'マルチシグエスクロー保護による安全な支払い',
  },

  profile: {
    title: 'プロフィール',
    myProfile: 'マイプロフィール',
    editProfile: 'プロフィールを編集',
    about: '自己紹介',
    location: '場所',
    joined: '{{date}}に参加',
    followers: 'フォロワー',
    following: 'フォロー中',
    listings: '出品',
    reviews: 'レビュー',
    verified: '認証済み',
    store: 'ストア',
    otc: 'OTC',
    posts: '投稿',
    community: 'コミュニティ',
    viewStore: 'ストアを見る',
    follow: 'フォロー',
    unfollow: 'フォロー解除',
    message: 'メッセージ',
    block: 'ブロック',
    report: '報告',
    myStore: 'マイストア',
    addNewListing: '新規出品',
    contactInformation: '連絡先',
    email: 'メール',
    phone: '電話',
    website: 'ウェブサイト',
    profileInformation: 'プロフィール情報',
    name: '名前',
    bio: '自己紹介',
    saveChanges: '変更を保存',
    noProfileData: 'プロフィールデータを読み込めませんでした',
  },

  settings: {
    title: '設定',
    general: '一般',
    account: 'アカウント',
    notifications: '通知',
    privacy: 'プライバシー',
    security: 'セキュリティ',
    language: '言語',
    currency: '通貨',
    theme: 'テーマ',
    darkMode: 'ダークモード',
    lightMode: 'ライトモード',
    system: 'システム',
    logout: 'ログアウト',
    deleteAccount: 'アカウント削除',
    backup: 'バックアップ',
    restore: '復元',
    about: '概要',
    version: 'バージョン',
    termsOfService: '利用規約',
    privacyPolicy: 'プライバシーポリシー',
    appearance: '外観',
    chooseTheme: 'テーマを選択',
    displayMode: '表示モード',
    avatar: 'アバター',
    loadAvatar: 'アバターを変更',
    loadHeader: 'カバーを変更',
  },

  theme: {
    classic: 'クラシックシアン',
    classicDesc: 'Mobazhaブランドのクラシックカラー、爽やかで活気ある',
    crypto: 'クリプトナイト',
    cryptoDesc: 'ネオングリーンの暗い背景、暗号トレーダー向け',
    business: 'ビジネスブルー',
    businessDesc: 'プロフェッショナルなブルートーン、ビジネス向け',
    cyberpunk: 'サイバーパンク',
    cyberpunkDesc: '紫ピンクのグラデーション、NFTやデジタルアートに最適',
    nature: 'トラストオアシス',
    natureDesc: 'フォレストグリーン、信頼と持続可能性を伝える',
    luxury: 'ラグジュアリーゴールド',
    luxuryDesc: 'ブラックとゴールド、高級品取引向け',
  },

  errors: {
    generic: 'エラーが発生しました',
    network: 'ネットワークエラー。接続を確認してください。',
    notFound: '見つかりません',
    unauthorized: 'ログインしてください',
    forbidden: 'アクセスが拒否されました',
    serverError: 'サーバーエラー。後でもう一度お試しください。',
    timeout: 'リクエストがタイムアウトしました',
    offline: 'オフラインです',
    invalidInput: '無効な入力',
    requiredField: 'この項目は必須です',
  },

  time: {
    justNow: 'たった今',
    minutesAgo: '{{count}}分前',
    hoursAgo: '{{count}}時間前',
    daysAgo: '{{count}}日前',
    weeksAgo: '{{count}}週間前',
    monthsAgo: '{{count}}ヶ月前',
    yearsAgo: '{{count}}年前',
  },

  hero: {
    badge: '分散型 & P2P',
    titleLine1: '自由に取引。',
    titleLine2: '安全に取引。',
    subtitle:
      'デジタル商品、サービス、暗号通貨OTC取引のための真の分散型マーケットプレイス。仲介者なし。検閲なし。',
    exploreMarket: 'マーケットを探索',
    startSelling: '販売を開始',
    activeStores: 'アクティブストア',
    productsListed: '出品商品',
    platformFee: 'プラットフォーム手数料',
  },

  footer: {
    tagline: '未来のコマースのための分散型マーケットプレイス。',
    marketplace: 'マーケットプレイス',
    browseProducts: '商品を閲覧',
    categories: 'カテゴリー',
    findModerators: 'モデレーターを探す',
    startSelling: '販売を開始',
    resources: 'リソース',
    gettingStarted: 'はじめに',
    documentation: 'ドキュメント',
    api: 'API',
    faq: 'よくある質問',
    company: '会社',
    about: '会社概要',
    blog: 'ブログ',
    careers: '採用情報',
    contact: 'お問い合わせ',
    allRightsReserved: 'All rights reserved.',
  },

  marketplace: {
    title: 'コミュニティマーケットプレイス',
    subtitle: 'コミュニティ主導のマーケットプレイスを発見して参加しましょう。',
    searchPlaceholder: 'マーケットプレイスを検索...',
    featured: '注目',
    featuredMarketplaces: '注目のマーケットプレイス',
    allMarketplaces: 'すべてのマーケットプレイス',
    searchResults: '検索結果',
    createMarketplace: 'マーケットプレイスを作成',
    members: 'メンバー',
    sellers: '販売者',
    products: '商品',
    noMarketplacesFound: 'マーケットプレイスが見つかりません',
    clearFilters: 'フィルターをクリア',
    sortByMembers: 'メンバー数順',
    sortByProducts: '商品数順',
    sortByName: 'アルファベット順',
    allCategories: 'すべてのカテゴリー',
  },

  moderator: {
    title: 'モデレーター',
    subtitle: '信頼できるモデレーターを選んでエスクローで取引を保護',
    searchPlaceholder: 'モデレーターを検索...',
    language: '言語',
    allLanguages: 'すべての言語',
    maxFee: '最大手数料 (%)',
    anyFee: '手数料制限なし',
    verifiedOnly: '認証済みのみ',
    sortBy: '並び替え',
    highestRating: '評価順',
    lowestFee: '手数料順',
    mostDisputes: '紛争処理数順',
    resetFilters: 'フィルターをリセット',
    moderatorsFound: '{{count}}人のモデレーターが見つかりました',
    disputesHandled: '{{count}}件の紛争を処理',
    success: '{{rate}}%成功',
    fee: '手数料',
    noModeratorsFound: '条件に合うモデレーターが見つかりません',
  },

  filter: {
    filters: 'フィルター',
    category: 'カテゴリー',
    priceRange: '価格範囲',
    min: '最小',
    max: '最大',
    rating: '評価',
    anyRating: '評価制限なし',
    stars: '{{count}}つ星以上',
    type: 'タイプ',
    allTypes: 'すべてのタイプ',
    physicalGoods: '物理商品',
    digitalGoods: 'デジタル商品',
    services: 'サービス',
    rwaTokens: 'RWAトークン',
  },

  empty: {
    noProductsFound: '商品が見つかりません',
    noStoresFound: 'ストアが見つかりません',
    tryAdjustingSearch: '検索条件を調整してみてください。',
    tryAdjustingFilters: '検索やフィルターを調整してみてください。',
    noRecentSearches: '最近の検索はありません',
    loadMoreResults: 'さらに読み込む',
  },

  homeExtended: {
    trendingNow: '人気商品',
    trendingSubtitle: '今週の人気商品',
    featuredServices: '注目 & サービス',
    featuredSubtitle: '厳選された商品とサービス',
    browseCategories: 'カテゴリーを閲覧',
    electronics: '電子機器',
    digitalGoods: 'デジタル商品',
    services: 'サービス',
    cryptoOtc: '暗号通貨OTC',
  },

  searchExtended: {
    searchPlaceholder: '商品、ストアを検索、またはURLを貼り付け...',
    recentSearches: '最近の検索',
    clearAll: 'すべてクリア',
    popularCategories: '人気のカテゴリー',
    products: '商品',
    stores: 'ストア',
  },

  settingsExtended: {
    profile: 'プロフィール',
    country: '国',
    shippingAddresses: '配送先住所',
    manageAddresses: '配送先住所を管理',
    blockedUsers: 'ブロックしたユーザー',
    manageBlocked: 'ブロックしたアカウントを管理',
    pushNotifications: 'プッシュ通知',
    pushDescription: '注文やメッセージのプッシュ通知を受け取る',
    emailNotifications: 'メール通知',
    emailDescription: 'メールで更新を受け取る',
    store: 'ストア',
    privateStore: 'プライベートストア',
    privateStoreDesc: '承認されたユーザーのみが商品を閲覧可能',
    storePolicies: 'ストアポリシー',
    storePoliciesDesc: '返品ポリシー、利用規約',
    moderators: 'モデレーター',
    moderatorsDesc: '紛争モデレーターを管理',
    acceptedCrypto: '受け入れる暗号通貨',
    selected: '{{count}}件選択中',
    shippingOptions: '配送オプション',
    shippingOptionsDesc: '配送方法と料金を設定',
    advanced: '詳細設定',
    analytics: 'アナリティクス',
    analyticsDesc: '匿名の使用データを共有してアプリの改善に協力',
    backupWallet: 'ウォレットをバックアップ',
    backupWalletDesc: 'シードフレーズを保存',
    backupProfile: 'プロフィールをバックアップ',
    backupProfileDesc: 'プロフィールデータをエクスポート',
    restoreProfile: 'プロフィールを復元',
    restoreProfileDesc: 'バックアップからプロフィールをインポート',
    resyncTransactions: '取引を再同期',
    resyncDesc: 'ブロックチェーン取引を再同期',
    serverLogs: 'サーバーログ',
    serverLogsDesc: 'アプリケーションログを表示',
    checkForUpdates: '更新を確認',
    upToDate: '最新です',
    latestVersion: '最新バージョンを使用中です！',
    selectCountry: '国を選択',
    selectCurrency: '通貨を選択',
    currencyUpdated: '通貨が更新されました',
    acceptedCryptocurrencies: '受け入れる暗号通貨',
    done: '完了',
    light: 'ライト',
    dark: 'ダーク',
    restoreConfirmTitle: 'プロフィールを復元',
    restoreConfirmDesc:
      '復元しますか？先にバックアップを確認してください。この操作で現在のプロフィールデータが上書きされる可能性があります。',
    continue: '続行',
    logoutConfirmTitle: 'ログアウト',
    logoutConfirmDesc: 'ログアウトしますか？再度サインインする必要があります。',
    comingSoon: '近日公開',
    backupComingSoon: 'ウォレットバックアップ機能は近日公開！',
    restoreComingSoon: 'プロフィール復元機能は近日公開！',
    followingSystem: 'システムに従う',
    enabled: '有効',
    disabled: '無効',
  },

  userMenu: {
    myProfile: 'マイページ',
    myStore: 'マイストア',
    createListing: '商品を作成',
    sales: '販売注文',
    purchases: '購入注文',
    settings: '設定',
    logout: 'ログアウト',
  },

  // OTC プライベート取引
  otc: {
    title: 'OTC取引',
    createNft: 'NFT注文を作成',
    createErc3525: 'シェア注文を作成',
    createNftOrder: 'NFTプライベート注文を作成',
    createOrder: '注文を作成',
    noOtcYet: 'OTC資産がありません',
    noOtcInStore: 'このストアにはOTC資産がありません',
    noOtcInMarketplace: 'このマーケットにはOTC資産がありません',
    createFirstOtc: '最初のOTC注文を作成してプライベート取引を開始',
    beFirstToCreate: 'コミュニティで最初のOTC注文を作成しましょう',
    activeOrders: 'アクティブな注文',
    historyOrders: '履歴',
    fetchError: 'OTC注文の取得に失敗しました',
    step: { selectNft: 'NFTを選択', setPrice: '価格設定', shareLink: 'リンク共有' },
    status: { active: 'アクティブ', completed: '完了', cancelled: 'キャンセル' },
    price: '価格', priceDesc: '購入者はUSDTで支払います', shares: 'シェア', totalPrice: '合計', platformFeeNote: '* プラットフォーム手数料5%', youWillReceive: '受け取り金額', selectNftDesc: '販売するNFTを選択',
    creating: '作成中...', purchasing: '購入中...', cancelling: 'キャンセル中...', buyNow: '今すぐ購入', buyShares: 'シェアを購入', cancelOrder: '注文をキャンセル', confirmCancel: 'この注文をキャンセルしますか？',
    createFailed: '注文の作成に失敗しました', orderCreated: '注文が作成されました！', purchaseSuccess: '購入成功！', purchaseFailed: '購入失敗', cancelSuccess: '注文がキャンセルされました', cancelFailed: 'キャンセル失敗', orderNotFound: '注文が見つかりません', transactionCompleted: '取引完了', orderCancelled: '注文キャンセル済み',
    shareText: 'プライベートNFTを販売中です、ぜひご覧ください！', shareToSell: '下のリンクを購入者に共有', shareToTelegram: 'Telegramで共有', viewOrder: '注文を見る', createAnother: '別の注文を作成',
    contractInfo: 'コントラクト情報', seller: '販売者', network: 'ネットワーク', expectedRevenue: '予想収益', weeklyRevenue: '週間収益', annualizedRevenue: '年間収益', chatWithSeller: '販売者に連絡',
    connectWalletRequired: 'ウォレットを接続してください', connectWalletDesc: 'OTC注文を作成するにはウォレット接続が必要です', backToStore: 'ストアに戻る',
    marketplaceOtcTitle: '🔐 プライベートOTC取引', marketplaceOtcDesc: 'コミュニティメンバーによるNFTとRWAシェアの出品',
  },

};

export default ja;
