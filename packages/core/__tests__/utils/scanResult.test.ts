...
const validateAddress: AddressValidator = (address, coinHint) => {
  ...
  if (coinHint === 'ZEC' && address === 't1historicaladdress') {
    return { coin: 'ZEC' };
  }
  return undefined;
};
...
describe('parseScanResult', () => {
  ...
  it('does not classify payment URIs outside the Community allowlist', () => {
    expect(parseScanResult('zcash:t1historicaladdress', { validateAddress })).toEqual({
      type: 'search',
      query: 'zcash:t1historicaladdress',
    });
  });
