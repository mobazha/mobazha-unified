...
interface RealOrderData {
  ...
  paymentState?: {
     ...
     * positive expected amount (Phase EVM-Safe v0.3.0 §4.2).
     */
    progress?: {
...
/** True when a cancel/refund settlement is confirmed on-chain (Safe) or recorded on legacy cancel msg. */
export function isRefundSettlementConfirmed(
  ...
  // Decline tx alone is ambiguous when Safe settlement surface exists (failed attempts also carry tx).
