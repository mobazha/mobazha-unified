...
export async function getOrderDetails(
  ...
  const mockFn = async () => {
    ...
    const settlementActions = undefined;
...
type CommunityOrderActionKind = 'confirm' | 'cancel' | 'complete' | 'dispute-release';

export async function executeSettlementAction(payload: {
  orderID: string;
  action: CommunityOrderActionKind;
  payoutAddress?: string;
}): Promise<SettlementActionResponse> {
  switch (payload.action) {
    case 'confirm':
      await confirmOrder({ orderID: payload.orderID, payoutAddress: payload.payoutAddress });
      break;
    case 'cancel':
      await cancelOrder({ orderID: payload.orderID });
      break;
    case 'complete':
      await completeOrder({ orderID: payload.orderID });
      break;
    case 'dispute-release':
      await claimPayment(payload.orderID);
      break;
  }
...
export async function getSettlementActionStatus(payload: {
  ...
  void payload;
  throw new Error('Asynchronous order settlement is not available in Community Edition');
}
