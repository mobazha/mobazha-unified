...
export async function createMarketplaceInviteLink(
  ...
  return hostingPost<MarketplaceInviteLink>(HOSTING_API.MARKETPLACE_INVITE_LINKS(marketplaceId), data);
}
...
export async function revokeMarketplaceInviteLink(
  ...
  return hostingDel<{ revoked: boolean }>(HOSTING_API.MARKETPLACE_INVITE_LINK(marketplaceId, linkId));
}
